...
const BrAmountApproverModal = ({ isOpen, toggle, budgetRequestAmount }) => {
  ...
  const validation = useFormik({
    ...
    validationSchema: Yup.object({
      bra_approved_amount: formattedAmountValidation(0, budgetRequestAmount?.bra_requested_amount, true),
      bra_source_government_approved: formattedAmountValidation(0, budgetRequestAmount?.bra_source_government_requested, true),
      bra_source_internal_approved: formattedAmountValidation(0, budgetRequestAmount?.bra_source_internal_requested, true),
      bra_source_support_approved: formattedAmountValidation(0, budgetRequestAmount?.bra_source_support_requested, true),
      bra_source_credit_approved: formattedAmountValidation(0, budgetRequestAmount?.bra_source_credit_requested, true),
    }),
    validateOnBlur: true,
          ...
          >
            <Row>
              <FormattedAmountField
                ...
                allowDecimal={true}
                infoText={`Requested Amount: ${parseFloat(budgetRequestAmount?.bra_requested_amount).toLocaleString()}`}
              />
              <div className="my-3 p-3">
                <CardTitle className="mb-3 bg-light p-2">
                  <h5 className="text-center">Budget Requested by Financing Source</h5>
                </CardTitle>
                <CardBody>
                  <Row className="gy-4">
                    <Col md={6} lg={3}>
                      <Card body className="shadow-sm border">
                        <FormattedAmountField
                          ...
                          className="mb-3"
                          allowDecimal={true}
                          infoText={`Requested Amount: ${parseFloat(budgetRequestAmount?.bra_source_government_requested).toLocaleString()}`}
                        />
                      </Card>
                    </Col>
                    <Col md={6} lg={3}>
                      <Card body className="shadow-sm border">
                        <FormattedAmountField
                          ...
                          className="mb-3"
                          allowDecimal={true}
                          infoText={`Requested Amount: ${parseFloat(budgetRequestAmount?.bra_source_internal_requested).toLocaleString()}`}
                        />
                      </Card>
                    </Col>
                    <Col md={6} lg={3}>
                      <Card body className="shadow-sm border">
                        <FormattedAmountField
                          ...
                          infoText={`Requested Amount: 
                            ${parseFloat(budgetRequestAmount?.bra_source_support_requested).toLocaleString()}
                            , Code: ${budgetRequestAmount?.bra_source_support_code}
                            `}
                        />
                      </Card>
                    </Col>
                    <Col md={6} lg={3}>
                      <Card body className="shadow-sm border">
                        <FormattedAmountField
                          ...
                          allowDecimal={true}
                          infoText={`Requested Amount:
                             ${parseFloat(budgetRequestAmount?.bra_source_credit_requested).toLocaleString()}
                             , Code: ${budgetRequestAmount?.bra_source_credit_code}
                            `}
                        />
                      </Card>
                    </Col>
            ...
            <Row>
              <Col>
                <div className="text-end">
                  ...
                  >
                    {updateBudgetRequestAmount.isPending && <Spinner size={"sm"} color="light" className="me-2" />}
                    {t("Approve")}
                  </Button>
                </div>
              </Col>
