...
import {
	...
	PROJECT_TYPES,
} from "./ProjectEvaluationConfig.js";
...
const ProjectEvaluationModel = (props) => {
	...
	// Mutation hooks
	const addProjectEvaluation = useAddProjectEvaluation();
	const updateProjectEvaluation =
		useUpdateProjectEvaluation();
	const deleteProjectEvaluation =
		useDeleteProjectEvaluation();
	...
	const validation = useFormik({
		...
		onSubmit: (values) => {
			...
			if (isEdit) {
				...
				handleUpdateProjectEvaluation(submissionData);
			} else {
				handleAddProjectEvaluation(submissionData);
			}
		},
	});

	// Handlers
	const handleAddProjectEvaluation = async (data) => {
		try {
			await addProjectEvaluation.mutateAsync(data);
	...
	const handleUpdateProjectEvaluation = async (data) => {
		try {
			await updateProjectEvaluation.mutateAsync(data);
	...
	const handleDeleteProjectEvaluation = async () => {
		if (projectMonitoringEvaluation?.mne_id) {
			try {
				await deleteProjectEvaluation.mutateAsync(
		...
		const commonProps = {
			...
			invalid:
				validation.touched[fieldName] && validation.errors[fieldName]
					...
					: false,
			disabled:
				addProjectEvaluation.isPending ||
				updateProjectEvaluation.isPending,
		};
	...
	return (
		<React.Fragment>
			...
			<DeleteModal
				show={deleteModal}
				onDeleteClick={handleDeleteProjectEvaluation}
				onCloseClick={() => setDeleteModal(false)}
				isLoading={deleteProjectEvaluation.isPending}
			/>
