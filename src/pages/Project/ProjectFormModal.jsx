 ...
 * @param {Array} clustersWithSectors - Array of clusters with sectors
 * @param {number} sectorId - The sector ID to find parent for
 * @returns {number|null} - Parent cluster ID or null if not found
 */
const getParentClusterIdFromSectorId = (clustersWithSectors, sectorId) => {
	...
	for (const cluster of clustersWithSectors) {
		if (cluster.children && Array.isArray(cluster.children)) {
			...
			if (foundSector) {
				return cluster.psc_id; // Return the cluster ID
			}
		}
	}

	return null; // Sector not found in any cluster
};

/**
 * Get the program and sub-program IDs for a specific output ID
 * @param {Array} programTreeData - The program tree data from API
 * @param {number} outputId - The output ID to find parents for
 * @returns {Object} - Object containing programId and subProgramId for the given output
 */
...
const ProjectFormModal = ({
	...
	// Render regional structure tab content
	const renderRegionalStructureTab = () => (
	...
	// Render form without tabs (for add mode)
	const renderFormWithoutTabs = () => renderBasicInfoTab();

	// Render form with tabs (for edit mode)
			...
			<Nav tabs className="nav-tabs-custom">
				...
				<NavItem>
					<NavLink
						...
						style={{ cursor: "pointer" }}
					>
						{t("Regional Structure")}
					</NavLink>
				</NavItem>
			</Nav>
			<TabContent activeTab={activeTab} className="p-3 border border-top-0">
				<TabPane tabId="1">{renderBasicInfoTab()}</TabPane>
				<TabPane tabId="2">{renderRegionalStructureTab()}</TabPane>
			</TabContent>
		</>
	);
