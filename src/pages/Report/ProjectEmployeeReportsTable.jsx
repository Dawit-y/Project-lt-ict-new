...
const ProjectEmployeeReportsTable = ({
  data,
  isGlobalFilter,
  SearchPlaceholder,
  t,
  ...
  const totalProjects = new Set(data.map((item) => item["Project Name(Code)"]))
    .size;
    ...
    <div className="table-responsive">
            ...
            page.map((row) => {
              ...
              return (
                <React.Fragment key={row.id}>
                  {row.original.isProjectRow ? (
                    <tr>
                      <td rowSpan={row.original.rowSpan} className="fw-bold">
                        {row.original.sn}
                      </td>
                      <td rowSpan={row.original.rowSpan} className="fw-bold">
                      ...
                      <td rowSpan={row.original.rowSpan} className="fw-bold">
                        {row.original.woreda || "N/A"}
                      </td>
                      <td rowSpan={row.original.rowSpan} className="fw-bold">
                        {row.original.sector || "N/A"}
                      </td>
                      <td>
                        {row.original.firstEmployee?.["Employee Name"] || "N/A"}
                      </td>
                      <td>
                        {row.original.firstEmployee?.["Start Date"]
                          ? new Date(
                              row.original.firstEmployee["Start Date"],
                            ).toLocaleDateString()
                          : "N/A"}
                      ...
                      <td>
                        {row.original.firstEmployee?.["End Date"]
                          ? new Date(
                              row.original.firstEmployee["End Date"],
                            ).toLocaleDateString()
                          : "N/A"}
      ...
      <div className="d-flex justify-content-between align-items-center mt-3">
        <div>
          ...
          {Math.min((pageIndex + 1) * pageSize, flatData.length)} of{" "}
          {flatData.length} entries
        </div>
        ...
        <Pagination>
          ...
          {Array.from({ length: Math.min(5, pageOptions.length) }, (_, i) => {
            const pageNumber =
              Math.max(0, Math.min(pageOptions.length - 5, pageIndex - 2)) + i;
            return (
              <PaginationItem
