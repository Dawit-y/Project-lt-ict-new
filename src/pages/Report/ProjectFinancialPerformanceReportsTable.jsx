...
const FinancialProjectsTable = ({
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((d) =>
      (d.sector && d.sector.toLowerCase().includes(lower)) ||
      (d.prj_name && d.prj_name.toLowerCase().includes(lower)) ||
      (d.prj_code && d.prj_code.toLowerCase().includes(lower))
    );
  }, [data, searchTerm]);
  ...
  const groupedData = useMemo(() => {
    ...
    filteredData.forEach((item) => {
      const sector = item.sector || t("Other Sector");
      const projKey = `${item.prj_code}_${item.prj_name}`;

      if (!sectors[sector]) sectors[sector] = {};
      if (!sectors[sector][projKey]) {
        sectors[sector][projKey] = {
          prj_code: item.prj_code,
          prj_name: item.prj_name,
          entries: [],
          projectNumber: projectCounter++,
        };
      }

      sectors[sector][projKey].entries.push(item);
    });
  ...
  let projectSerial = 1;
    ...
    Object.entries(groupedData).forEach(([sectorName, projects]) => {
      ...
      Object.values(projects).forEach((proj) => {
        proj.entries.forEach((entry, i) => {
          rows.push({
            type: "project",
            ...entry,
            prj_code: i === 0 ? proj.prj_code : "",
            prj_name: i === 0 ? proj.prj_name : "",
            indexLabel: i === 0 ? projectSerial++ : "",
          });
        });
      });
    });
  ...
  return (
    <div style={{ width: "100%", overflowX: "auto" }}>
      ...
      <div style={{ overflowX: "auto", width: "100%", border: "1px solid #dee2e6", borderRadius: "4px", marginBottom: "1rem" }}>
        <table className={`table table-bordered table-hover ${tableClass}`} style={{ width: "100%", fontSize: "0.85rem", minWidth: "1000px" }}>
          <thead className={theadClass} style={{ position: "sticky", top: 0, backgroundColor: "#f8f9fa", zIndex: 1 }}>
            ...
            <tr>
              <th>{t("Sep")}</th><th>{t("Oct")}</th><th>{t("Nov")}</th><th>{t("Sum")}</th>
              <th>{t("Dec")}</th><th>{t("Jan")}</th><th>{t("Feb")}</th><th>{t("Sum")}</th>
              <th>{t("Mar")}</th><th>{t("Apr")}</th><th>{t("May")}</th><th>{t("Sum")}</th>
              <th>{t("Jun")}</th><th>{t("Jul")}</th><th>{t("Aug")}</th><th>{t("Sum")}</th>
            </tr>
          </thead>
          <tbody>
            {currentRows.length > 0 ? (
              currentRows.map((row, index) => {
                if (row.type === "sector") {
                  return (
                    <tr key={`sector-${row.sectorName}`} 
                    style={{ backgroundColor: "#e9ecef", fontWeight: "bold", fontSize: "0.9rem" }}>
