...
const BudgetExpenditureTable = ({ data, isGlobalFilter, SearchPlaceholder, t }) => {
  ...
  const groupedData = React.useMemo(() => {
    ...
    data.forEach(item => {
      ...
      const amount = Number(String(item['Budget Amount']).replace(/[^0-9.-]+/g, "")) || 0;
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="d-flex flex-column">
            {row.original.expenditures.map((exp, i) => (
              <div key={i}>
                {exp.date === 'N/A' ? 'N/A' : new Date(exp.date).toLocaleDateString()}
              </div>
            ))}
          </div>
        )
