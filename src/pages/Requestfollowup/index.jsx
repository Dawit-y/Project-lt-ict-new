...
const RequestFollowupModel = ({ request, isActive }) => {
	...
	const { data, isLoading, error, isError, isFetching, refetch } =
		useSearchRequestFollowups(param, isValidParam);
	...
	const validation = useFormik({
		...
		initialValues: {
			...
			rqf_forwarding_dep_id:
				(requestFollowup && requestFollowup.rqf_forwarding_dep_id) || "",
			rqf_forwarded_to_dep_id:
				(requestFollowup && requestFollowup.rqf_forwarded_to_dep_id) || "",
			rqf_forwarding_date:
				(requestFollowup && requestFollowup.rqf_forwarding_date) || "",
			rqf_received_date:
				(requestFollowup && requestFollowup.rqf_received_date) || "",
			rqf_description:
				(requestFollowup && requestFollowup.rqf_description) || "",
	...
	const isAddAllowed = () => {
		...
		if (
			data?.data?.find((item) => item.rqf_forwarding_dep_id === departmentId)
		) {
			return false;
		}
		...
		if (
			parseInt(request?.bdr_request_status) === 3 ||
			parseInt(request?.bdr_request_status) === 4
		) {
			return false;
		}
		return true;
				...
				cell: (cellProps) => {
					return (
						<div className="d-flex gap-1">
							{cellProps.row.original.rqf_forwarding_dep_id == departmentId &&
								!request.forwarded && (
									<Button
										color="Link"
				...
				<div className="">
					...
					<Modal isOpen={modal} toggle={toggle} className="modal-xl">
						<ModalHeader toggle={toggle} tag="h4">
							{!!isEdit
								? t("edit") + " " + t("request_followup")
								: t("add") + " " + t("request_followup")}
						</ModalHeader>
						<ModalBody>
							<Form
								onSubmit={(e) => {
