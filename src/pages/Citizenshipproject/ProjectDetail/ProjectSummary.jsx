...
const ProjectSummary = ({ data }) => {
  ...
  const additionalBudget =
    parseFloat(data?.additional_budget?.additional_budget) || 0;

  const budgetData = {
    totalBudget:
      parseFloat(data?.data?.prj_total_estimate_budget) + additionalBudget || 0,
  ...
  if (projectData.status_id >= 5) {
    startDate = projectData.prj_start_date_gc
      ? new Date(projectData.prj_start_date_gc)
      : null;
  } else {
    startDate = projectData.prj_start_date_plan_gc
      ? new Date(projectData.prj_start_date_plan_gc)
      : null;
  }

  const endDate = projectData.prj_end_date_plan_gc
    ? new Date(projectData.prj_end_date_plan_gc)
    : null;
      ...
      <CardBody>
        <Row>
          ...
          <Col md={4}>
            ...
            <h6>
              {t("Current Total Budget")}:{" "}
              {budgetData.totalBudget.toLocaleString()} {t("birr")}
            </h6>
            <h6>
              {t("Additional Budget")}: {additionalBudget.toLocaleString()}{" "}
              {t("birr")}
            </h6>
            <h6>
              {t("Used Budget")}: {budgetData.usedBudget.toLocaleString()}{" "}
              {t("birr")}
            </h6>
            <h6>
              {t("Remaining Amount")}:{" "}
              {budgetData.remainingAmount.toLocaleString()} {t("birr")}
            </h6>
            <div>
              <Chart
          ...
          <Col md={4}>
            ...
            {timeData ? (
              <>
                <h6>
                  {t("Total Project Duration")}: {timeData.totalDuration}{" "}
                  {t("days")}
                </h6>
                <h6>
                  {t("Used Time")}: {timeData.usedTime} {t("days")}
                </h6>
                <h6>
                  {t("Remaining Time")}: {timeData.remainingTime} {t("days")}
                </h6>
                <div>
                  <Chart
