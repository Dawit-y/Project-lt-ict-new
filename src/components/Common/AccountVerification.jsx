...
const AccountVerification = () => {
	...
	const { data: csoData, isLoading: isCsoInfoLoading } = useSearchCsoInfos(
		param,
		isValidParam
	);
	const csoId = csoData?.data?.length > 0 ? csoData.data[0].cso_id : null;
	const csoStatus =
		csoData?.data?.length > 0 ? csoData.data[0].cso_status : null;

	const docParams = { prd_owner_type_id: PAGE_ID.CSO, prd_owner_id: csoId };
	const isQueryEnabled = Object.values(docParams).every(
		(value) => value !== null && value !== undefined
	);
				...
				<Container className="">
					<div className="mb-5">
						<div className="text-center pt-5">
							<h2 className="">
								Dear {csoData?.data?.length > 0 ? csoData.data[0].cso_name : ""}
								, Operational Agreement is Required
							</h2>
							<p className="lead">
								Before you proceed with proposal submission, Operational
							...
							{isLoading ? (
								...
								<>
									<div className="alert alert-success mb-4">
										...
										You've submitted {data.data.length} document(s). Our team is
										reviewing them.
									</div>
									<FileList
										files={data?.data || []}
										...
										handleDeleteProjectDocument={handleDeleteProjectDocument}
