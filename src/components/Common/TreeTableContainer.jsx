...
const TreeTableContainer = ({ data, columns, setData }) => {
  ...
  const handleDragEnd = async (event) => {
    ...
    const targetZone = overNode.level === 'zone'
      ? overNode
      : findParentZone(data, overNode.id);
    ...
    <>
      ...
      <Modal isOpen={confirmModal} toggle={handleCancel}>
        ...
        <ModalBody>
          {dragInfo && (
            <>
              <p>Are you sure you want to move <strong>{dragInfo.woreda.name}</strong> from <strong>{dragInfo.oldZone.name}</strong> to <strong>{dragInfo.newZone.name}</strong>?</p>
        ...
        <ModalFooter>
          ...
          <Button color="primary" onClick={confirmMove}>
            {updateFolder.isPending ? <Spinner size={"sm"} className='me-2' /> : ""}
            Confirm Move
          </Button>
        </ModalFooter>
      </Modal>
...
const DraggableRow = ({ row, isDraggable, isDroppable }) => {
  ...
  const style = {
    ...
    position: 'relative',
    backgroundColor: isDragging ? '#f8f9fa' : droppable.isOver ? '#e2f0ff' : 'inherit',
  };
